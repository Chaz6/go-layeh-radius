...
type ContextName uint32

var ContextName_Strings = map[ContextName]string{}

func (a ContextName) String() string {
	if str, ok := ContextName_Strings[a]; ok {
		return str
	}
	return "ContextName(" + strconv.FormatUint(uint64(a), 10) + ")"
}

func ContextName_Add(p *radius.Packet, value ContextName) (err error) {
	a := radius.NewInteger(uint32(value))
	return _Zte_AddVendor(p, 4, a)
}

func ContextName_Get(p *radius.Packet) (value ContextName) {
	value, _ = ContextName_Lookup(p)
	return
}

func ContextName_Gets(p *radius.Packet) (values []ContextName, err error) {
	var i uint32
	for _, attr := range _Zte_GetsVendor(p, 4) {
		...
		values = append(values, ContextName(i))
	}
	return
}

func ContextName_Lookup(p *radius.Packet) (value ContextName, err error) {
...
func ContextName_Set(p *radius.Packet, value ContextName) (err error) {
	a := radius.NewInteger(uint32(value))
